...
func (r *StarRepositoryTestSuite) SetupSuite() {
	...
	r.assetRepository, err = postgres.NewAssetRepository(r.client, r.userRepository, postgres.AssetRepositoryConfig{
		DefaultGetMaxSize: postgres.DEFAULT_MAX_RESULT_SIZE,
		Logger:            logger,
	})
	if err != nil {
		r.T().Fatal(err)
