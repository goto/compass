...
func (_m *DeadJobManager) ClearDeadJobs(ctx context.Context, jobIDs []string) error {
	...
	if len(ret) == 0 {
		panic("no return value specified for ClearDeadJobs")
	}
...
func (_m *DeadJobManager) DeadJobs(ctx context.Context, size int, offset int) ([]worker.Job, error) {
	...
	if len(ret) == 0 {
		panic("no return value specified for DeadJobs")
	}
...
func (_m *DeadJobManager) Resurrect(ctx context.Context, jobIDs []string) error {
	...
	if len(ret) == 0 {
		panic("no return value specified for Resurrect")
	}
...
func NewDeadJobManager(t interface {
	mock.TestingT
	Cleanup(func())
}) *DeadJobManager {
	mock := &DeadJobManager{}
